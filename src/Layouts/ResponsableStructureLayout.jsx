...
import GestionProjets from "../Components/responsable_structure/GestionProjet";

export const ResponsableStructureLayout = () => {
  ...
  useEffect(() => {
    const storedStructureName = localStorage.getItem('structure_name');
    const storedRegionName = localStorage.getItem('region_name');
    if (storedStructureName) setStructureName(storedStructureName);
    if (storedRegionName) setRegionName(storedRegionName);
  }, []);

  const mainMenuItems = [
    { name: "Dashboard", icon: <DashboardIcon />, path: "/responsable-structure/dashboard" },
    { name: "Equipe", icon: <GroupesIcon />, path: "/responsable-structure/equipe" },
    { name: "Activités", icon: <ParametersIcon />, path: "/responsable-structure/activites" },
  ];

  const projectMenuItems = [
    { name: "Projets", icon: <ThemesIcon />, path: "/responsable-structure/projets" },
    { name: "Recap", icon: <ThemesIcon />, path: "/responsable-structure/recap" },
    { name: "Classification", icon: <ClassificationIcon />, path: "/responsable-structure/classification" },
    { name: "Risque Analyse", icon: <RisqueIcon />, path: "/responsable-structure/risque" },
    { name: "Rapport", icon: <RapportIcon />, path: "/responsable-structure/rapport" },
  ];

  const settingsMenuItems = [
    { name: "Profile", icon: <ProfileIcon />, path: "/responsable-structure/profile" },
    { name: "Notification", icon: <NotifIcon />, path: "/responsable-structure/notification" },
  ];
    ...
    <NavLink
      ...
      className={({ isActive }) => 
        `flex items-center gap-3 px-6 py-3 text-gray-600 hover:bg-[#FF8500] hover:text-white transition-all duration-200 ${
          isActive || activeMainMenu === item.name ? 'bg-yellow-100 text-yellow-700 border-l-4 border-yellow-500' : ''
        }`
      }
      end
    >
      {({ isActive }) => (
        <>
          <span className={`w-5 h-5 flex items-center justify-center [&>svg]:w-full [&>svg]:h-full [&>svg]:fill-current ${
            isActive || activeMainMenu === item.name ? 'text-yellow-600' : 'text-gray-700'
          }`}>
            {item.icon}
          </span>
      ...
      <aside className="bg-white text-gray-800 h-screen sticky top-0 flex flex-col border-r border-gray-200 shadow-sm">
        <div className="px-6 py-5 border-b border-gray-200">
          <h2 className="text-xl font-bold">
            <span className="text-yellow-600">Sonatrach</span>
            <span className="text-orange-500">Budget</span>
          </h2>
          {regionName && (
            <p className="text-xs text-gray-500 mt-1">
              <span className="font-medium">Région:</span> {regionName}
            </p>
          )}
          {structureName && (
            <p className="text-xs text-gray-500">
              <span className="font-medium">Structure:</span> {structureName}
            </p>
          )}
        </div>
