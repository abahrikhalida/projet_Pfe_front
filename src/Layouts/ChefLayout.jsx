...
export const ChefLayout = () => {
  ...
  useEffect(() => {
    ...
    setUserInfo({ role, regionId, structureId });
  }, []);
  ...
  const getMenuItems = () => {
    ...
    const commonMenu = [
      { name: "Dashboard", icon: <DashboardIcon />, path: `/${role}/dashboard` },
    ];

    const projectsMenu = [
      { name: "Projets", icon: <ThemesIcon />, path: `/${role}/projets` },
      { name: "Recap", icon: <ThemesIcon />, path: `/${role}/recap` },
      { name: "Classification", icon: <ClassificationIcon />, path: `/${role}/classification` },
      { name: "Risque Analyse", icon: <RisqueIcon />, path: `/${role}/risque` },
      { name: "Rapport", icon: <RapportIcon />, path: `/${role}/rapport` },
    ];

    const settingsMenu = [
      { name: "Profile", icon: <ProfileIcon />, path: `/${role}/profile` },
      { name: "Notification", icon: <NotifIcon />, path: `/${role}/notification` },
    ];
    ...
    switch (role) {
      ...
      case 'directeur_region':
        specificMenu = [
          { name: "Structures", icon: <GroupesIcon />, path: "/directeur-region/structures" },
          { name: "Responsables", icon: <GroupesIcon />, path: "/directeur-region/responsables" },
          { name: "Paramètres", icon: <ParametersIcon />, path: "/directeur-region/parametres" },
        ];
        break;
      ...
      case 'agent':
        specificMenu = [
          { name: "Mes tâches", icon: <ParametersIcon />, path: "/agent/taches" },
          { name: "Mon profil", icon: <ProfileIcon />, path: "/agent/profile" },
        ];
        break;
      default:
        specificMenu = [];
    ...
    return {
      mainMenu: [...commonMenu, ...specificMenu],
      projectsMenu: projectsMenu,
      settingsMenu: settingsMenu
    };
  };

  const { mainMenu, projectsMenu, settingsMenu } = getMenuItems();
  ...
  return (
    <div className="grid grid-cols-[220px_1fr] min-h-screen">
      <aside className="bg-white text-gray-800 h-screen sticky top-0 flex flex-col border-r border-gray-200 shadow-sm">
        <h2 className="text-xl font-bold px-6 py-5">
          <span className="text-blue-600">Sonatrach</span>
          <span className="text-orange-500">Budget</span>
        </h2>
        
        <nav className="flex-1 overflow-y-auto scrollbar-thin">
          <div className="flex flex-col py-2">
            {mainMenu.map((item, idx) => renderMenuItem(item, idx))}
          </div>

          <div className="flex flex-col py-2 mt-auto border-t border-gray-200 pt-4">
            {projectsMenu.map((item, idx) => renderMenuItem(item, `project-${idx}`))}
          </div>
       
          <div className="flex flex-col py-2 mt-auto border-t border-gray-200 pt-4">
            {settingsMenu.map((item, idx) => renderMenuItem(item, `settings-${idx}`))}
          </div>
