...
export const AgentLayout = () => {
  ...
  const mainMenuItems = [
    { name: "Dashboard", icon: <DashboardIcon />, path: "/agent/dashboard" },
    { name: "Mes Tâches", icon: <ParametersIcon />, path: "/agent/taches" },
    { name: "Mon Profil", icon: <ProfileIcon />, path: "/agent/profile" },
  ];

  const projectMenuItems = [
    { name: "Projets", icon: <ThemesIcon />, path: "/agent/projets" },
    { name: "Recap", icon: <ThemesIcon />, path: "/agent/recap" },
    { name: "Classification", icon: <ClassificationIcon />, path: "/agent/classification" },
    { name: "Risque Analyse", icon: <RisqueIcon />, path: "/agent/risque" },
    { name: "Rapport", icon: <RapportIcon />, path: "/agent/rapport" },
  ];

  const settingsMenuItems = [
    { name: "Notification", icon: <NotifIcon />, path: "/agent/notification" },
  ];
    ...
    <NavLink
      ...
      className={({ isActive }) => 
        `flex items-center gap-3 px-6 py-3 text-gray-600 hover:bg-[#FF8500] hover:text-white transition-all duration-200 ${
          isActive || activeMainMenu === item.name ? 'bg-purple-100 text-purple-700 border-l-4 border-purple-500' : ''
        }`
      }
      end
    >
      {({ isActive }) => (
        <>
          <span className={`w-5 h-5 flex items-center justify-center [&>svg]:w-full [&>svg]:h-full [&>svg]:fill-current ${
            isActive || activeMainMenu === item.name ? 'text-purple-600' : 'text-gray-700'
          }`}>
            {item.icon}
          </span>
      ...
      <aside className="bg-white text-gray-800 h-screen sticky top-0 flex flex-col border-r border-gray-200 shadow-sm">
        <div className="px-6 py-5 border-b border-gray-200">
          <h2 className="text-xl font-bold">
            <span className="text-purple-600">Sonatrach</span>
