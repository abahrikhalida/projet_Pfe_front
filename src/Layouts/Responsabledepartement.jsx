// ResponsableDepartementLayout.jsx
...
export const ResponsableDepartementLayout = () => {
  ...
  useEffect(() => {
    const fetchInfo = async () => {
      ...
      try {
        ...
        if (departementId) {
          try {
            const deptRes = await axiosInstance.get(`/params/departements/id/${departementId}`);
            const departement = deptRes.data.data || deptRes.data;
            setDepartementName(departement.nom_departement || 'Département');
          } catch (error) {
            console.error("Erreur chargement département:", error);
            setDepartementName("Département");
          }
        }
        ...
        if (directionId) {
          try {
            const dirRes = await axiosInstance.get(`/params/directions/${directionId}`);
            const direction = dirRes.data.data || dirRes.data;
            setDirectionName(direction.nom_direction || 'Direction');
          } catch (error) {
  ...
  const mainMenuItems = [
    { name: "Dashboard", icon: <DashboardIcon />, path: "/responsable-departement/dashboard" },
    { name: "Agents", icon: <GroupesIcon />, path: "/responsable-departement/agents" },
    { name: "Projets", icon: <ThemesIcon />, path: "/responsable-departement/projets" },
    { name: "Paramètres", icon: <ParametersIcon />, path: "/responsable-departement/parametres" },
  ];

  const projectMenuItems = [
    { name: "Classification", icon: <ClassificationIcon />, path: "/responsable-departement/classification" },
    { name: "Risque Analyse", icon: <RisqueIcon />, path: "/responsable-departement/risque" },
    { name: "Rapport", icon: <RapportIcon />, path: "/responsable-departement/rapport" },
  ];

  const settingsMenuItems = [
    { name: "Profile", icon: <ProfileIcon />, path: "/responsable-departement/profile" },
    { name: "Notification", icon: <NotifIcon />, path: "/responsable-departement/notification" },
  ];
    ...
    <NavLink
      ...
      className={({ isActive }) => 
        `flex items-center gap-3 px-6 py-3 text-gray-600 hover:bg-[#FF8500] hover:text-white transition-all duration-200 ${
          isActive || activeMainMenu === item.name ? 'bg-green-100 text-green-700 border-l-4 border-green-500' : ''
        }`
      }
      end
    >
      {({ isActive }) => (
        <>
          <span className={`w-5 h-5 flex items-center justify-center [&>svg]:w-full [&>svg]:h-full [&>svg]:fill-current ${
            isActive || activeMainMenu === item.name ? 'text-green-600' : 'text-gray-700'
          }`}>
            {item.icon}
          </span>
            ...
            <>
              {directionName && (
                <p className="text-xs text-gray-500 mt-1">
                  <span className="font-medium">Direction:</span> {directionName}
                </p>
              )}
              {departementName && (
                <p className="text-xs text-gray-500">
                  <span className="font-medium">Département:</span> {departementName}
                </p>
              )}
            </>
          )}
