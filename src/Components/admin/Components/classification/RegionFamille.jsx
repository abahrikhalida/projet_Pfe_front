...
// Modal Component pour Region Detail
      ...
      >
        <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 rounded-t-2xl flex justify-between items-center z-10">
          <div>
            ...
            <p className="text-[#FF8500] font-semibold text-base">{region.region_nom}</p>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:bg-gray-100 rounded-full p-2 transition-colors">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {section.columns.map((col) => (
                  <div key={col.key} className="bg-[#FAFAFA] border border-gray-100 rounded-xl px-4 py-3 hover:border-[#FF8500]/30 hover:bg-[#FFF9F0] transition-colors">
                    ...
                    <p className="text-sm font-semibold text-gray-800">{fmt(region.total_region[col.key])}</p>
                  </div>
                ))}
              </div>
            </div>
...
const RegionRow = ({ region, onFamilleClick, onRegionDetailClick }) => {
      ...
      >
        <td className="py-2 px-4 align-middle">
          <div className="flex items-center gap-2 min-h-[32px]">
            ...
            <div>
              <div className="text-sm font-semibold text-gray-800">{region.region_nom}</div>
              <div className="text-xs text-gray-400 mt-0.5">({region.familles.length} famille{region.familles.length !== 1 ? 's' : ''})</div>
            </div>
          </div>
        </td>
        {baseColumns.map((col) => (
  ...
  return (
    <div className="bg-white rounded-[20px] shadow-sm border border-gray-100 p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <span className="text-sm text-gray-500">{regions.length} région{regions.length !== 1 ? 's' : ''}</span>
      </div>
        ...
        <table className="w-full table-fixed">
          <thead>
            <tr className="bg-gradient-to-r from-[#F9F9F9] to-[#F0F0F0] border-b border-[#E4E4E4]">
              <th className="w-[20%] py-2 px-4 text-left text-sm font-semibold text-[#4A4A4A] rounded-tl-lg">Région / Famille</th>
              {baseColumns.map((col) => (
          ...
          <tbody>
            {regions.map((region, i) => (
              <RegionRow 
                ...
                onRegionDetailClick={(region) => setSelectedRegionDetail(region)}
              />
            ))}
          </tbody>
      ...
      {selectedFamille && (
        <FamilleModal famille={selectedFamille} regionNom={selectedRegion} onClose={() => { setSelectedFamille(null); setSelectedRegion(""); }} />
      )}

      {selectedRegionDetail && (
        <RegionDetailModal region={selectedRegionDetail} onClose={() => setSelectedRegionDetail(null)} />
      )}
